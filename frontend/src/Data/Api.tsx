...
export async function getMyBorrowBook(
  token: string,
  params: Record<string, number>
): Promise<BorrowType[] | null> {
...
export async function fetchUpdateBook(
  data: UpdateBookType,
  token: string,
  id: string | number
): Promise<void> {
  ...
  if (data.publishedDate)
    formData.append("publishedDate", String(data.publishedDate));
...
export async function fetchCreateBook(
  endPoint: string,
  data: CreateBookType,
  token: string
) {
...
export async function fetchDeleteBook(
...
export async function getUserProfile(token: string): Promise<User> {
  ...
  if (repsonse.status !== HttpStatusCode.Ok)
    throw new AxiosError("Token không hợp lệ!");
...
export async function getTokenPayload(
  ...
  if (reponse.status !== HttpStatusCode.Ok)
    throw new AxiosError("Token không hợp lệ, vui lòng đăng nhập lại");
...
export async function fetchRegisterUser(endpoint: string, data: RegisterType) {
  ...
  if (response.status !== HttpStatusCode.Created)
    throw new AxiosError("Đăng kí thất bại!");
  return response.data;
}
